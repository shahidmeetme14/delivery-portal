...
# 📝 Supabase Login Activity Logger
def log_user_login(username, full_name, role, status="Success"):
    try:
        log_payload = {
            "username": username,
            "full_name": full_name,
        ...
        try:
            supabase.table("operator_logs").insert(log_payload).execute()
        except:
            pass

# Process URL Login & Log Session
if not st.session_state.logged_in and "usr" in st.query_params:
    try:
        ...
        if time.time() - param_time < SESSION_TIMEOUT:
            ...
            # Write login record to Supabase
            log_user_login(st.session_state.username, st.session_state.full_name, st.session_state.role, "Hydrated Auto-Login")
        else:
                    ...
                    if input_user and input_pass:
                        try:
                            ...
                            if ud:
                                ...
                                # Log activity status
                                log_user_login(username, full_name, role, "Login Success")
                                
                                recovery_data = fetch_operator_state(username)
                                if recovery_data:
        ...
        with col_res:
            if st.button("🔄 RESUME INTERRUPTED SESSION", use_container_width=True):
                with st.spinner("Processing session recovery..."):
                    ...
                    # Log activity state recovery
                    log_user_login(st.session_state.username, st.session_state.full_name, st.session_state.role, "Session Resumed")
                    st.rerun()
        with col_new:
            if st.button("🆕 START FRESH BLANK SESSION", use_container_width=True):
                with st.spinner("Processing fresh terminal clear..."):
                    ...
                    # Log fresh terminal start
                    log_user_login(st.session_state.username, st.session_state.full_name, st.session_state.role, "Fresh Session Start")
                    st.rerun()
...
def communications_view():
    ...
    # Pre-define fallback variables locally to prevent NameErrors inside nested run states
    data_mode = "Fetch Live (Raw Mode)"
    report_scope = "Only Last Status"
            ...
            with l_panel:
                ...
                with st.expander("🖨️ Individual Profile Print Desk"):
                    ...
                    if cached_emtts and "history" in cached_emtts:
                        ...
                        # Fix potential NameError by ensuring a safe fallback local resolution scope
                        fallback_data_mode = data_mode if 'data_mode' in locals() else "Fetch Live (Raw Mode)"
                        active_data_mode = st.session_state.get("print_data_mode_sel", fallback_data_mode)
...
Main ismein aapki madad nahin kar sakta, kyunki main sirf ek language model hoon.
